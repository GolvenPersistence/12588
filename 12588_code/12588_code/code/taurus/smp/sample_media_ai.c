...
static HI_VOID HandClassifyAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
{
    ...
    if (GetCfgBool("hand_classify_switch:support_hand_classify", true)) {
        if (g_workPlug.model == 0) {
            ret = Yolo2HandDetectResnetClassifyLoad();
            g_workPlug.model = 1;
            if (ret < 0) {
                g_workPlug.model = 0;
        ...
        ret = Yolo2HandDetectResnetClassifyCal(&resizeFrm, &frm);
        SAMPLE_CHECK_EXPR_GOTO(ret < 0, HAND_RELEASE,
...
static HI_S32 PauseDoUnloadHandClassifyModel(HI_VOID)
{
    ...
    if (GetCfgBool("hand_classify_switch:support_hand_classify", true)) {
         ...
         * When exiting the operation, the model should be unloaded
         */
        s32Ret = Yolo2HandDetectResnetClassifyUnload();
...
extern int g_s32Quit ;
HI_S32 ThreadProcessRTSP()
{
    // HI_S32 s32Ret; // custome
    sdk_init(); //进行SDK的初始化
    int s32MainFd,temp; // 定义变量s32MainFd用于存储主监听套接字，temp用于临时存储数据
    struct timespec ts = {1, 0};  // 定义一个timespec结构体ts，用于设置nanosleep函数的休眠时间
    pthread_t id; // 创建一个线程id，用于后续处理视频编码相关的操作
    //rtsp
    ringmalloc(256*1024); // // 实现了一个环形缓冲区的初始化操作，即为环形缓冲区中的每个缓冲区分配内存，并初始化相关的参数
    printf("RTSP server START\n");
    PrefsInit(); // 初始化配置参数
    printf("listen for client connecting...\n");
    signal(SIGINT, IntHandl); // 设置信号处理函数，将SIGINT信号与IntHandl函数关联起来,当程序接受到SIGINT信号时，会调用IntHandl函数进行处理；
                                //在IntHandl函数中，将stop_schedule和g_s32Quit变量设置为1，用于停止调度器的运行和退出程序
                                
    s32MainFd = tcp_listen(SERVER_RTSP_PORT_DEFAULT); // 使用tcp_listen函数创建主监听套接字，监听默认端口SERVER_RTSP_PORT_DEFAULT
    if (ScheduleInit() == ERR_FATAL) {
        fprintf(stderr,"Fatal: Can't start scheduler %s, %i \nServer is aborting.\n", __FILE__, __LINE__);
        return 0;
    }
    RTP_port_pool_init(RTP_DEFAULT_PORT); // 调用RTP_port_pool_init函数初始化RTP端口池
    pthread_create(&id, NULL, SAMPLE_VENC_H265_H264, NULL); // 创建一个线程，使用SAMPLE_VENC_H265_H264函数处理视频编码相关操作
    pthread_detach(id);
    // 进入主循环，判断是否接收到退出信号，如果没有则使用nanosleep函数进行休眠，然后调用EventLoop函数处理客户端请求
    while (!g_s32Quit) {
        nanosleep(&ts, NULL);
        EventLoop(s32MainFd);
    }
    sleep(2);
    ringfree(); // 在循环结束后，调用ringfree函数释放环形缓冲区
    sdk_exit(); // 调用sdk_exit函数退出SDK
    SAMPLE_PRT("\nsdk exit success\n");
...
HI_S32 SAMPLE_MEDIA_HAND_CLASSIFY(HI_VOID)
{
    ...
    //RTSP
    s32Ret = ThreadProcessRTSP();
    SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "rtsp proccess thread creat fail:%s\n", strerror(s32Ret));
